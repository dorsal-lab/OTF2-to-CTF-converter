...
OTF2_CallbackCode GlobalDef_CartTopology_callback(
    void* userData,
    ...
    const OTF2_CartDimensionRef* cartDimensions)
{
    ...
    if (numberOfDimensions > 0) {
        barectf_trace_GlobalDef_CartTopology(user_data->ctx, self, name,
...
OTF2_CallbackCode
GlobalDef_CartCoordinate_callback(void* userData,
    ...
    const uint32_t* coordinates)
{
    user_data_t* user_data = (user_data_t*)userData;
    if (numberOfDimensions > 0) {
        barectf_trace_GlobalDef_CartCoordinate(
            user_data->ctx, cartTopology, rank, numberOfDimensions, coordinates);
    }
    return OTF2_CALLBACK_SUCCESS;
}
